...
table.reg <- table.reg %>% dplyr::select(term, estimate, std.error, rating, bandwidth)
table.reg$term <- factor(table.reg$term,
                         levels = c('(Intercept)', 'treatTRUE', 'score'),
                         labels = c("(Intercept)", "Treatment", "Score"))
